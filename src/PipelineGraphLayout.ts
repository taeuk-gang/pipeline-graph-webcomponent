    ...
    for (const column of columns) {
        for (const row of column.rows) {
            for (const node of row) {
                ...
                if (node.isPlaceholder || (node as any).stage === column.topStage) {
                    continue;
                }
    ...
    for (const column of columns) {
        if (column.hasBranchLabels) {
            for (const row of column.rows) {
                ...
                if (!firstNode.isPlaceholder && (firstNode as any).stage.seqContainerName) {
                    labels.push({
                        x: column.startX,
                        y: firstNode.y,
                        key: `branchLabel-${++count}`,
                        node: firstNode,
                        text: (firstNode as any).stage.seqContainerName,
                    });
                }
            }
        }
